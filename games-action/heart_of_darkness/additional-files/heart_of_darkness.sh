...
REQ_FILES=('Dark_hod.lvl' 'Dark_hod.mst' 'Dark_hod.sss' 'Fort_hod.lvl' 
 ...
 'setup/Setup.dat')
DATA_PATH=`findpaths -e B_FIND_PATH_DATA_DIRECTORY hode | head -n 1`
SAVE_PATH=`/bin/finddir B_USER_NONPACKAGED_DATA_DIRECTORY`/hode/save

if [ -z $DATA_PATH ];
then
	DATA_PATH=`/bin/finddir B_USER_NONPACKAGED_DATA_DIRECTORY`/hode
...
do
    ...
    then
        if [ $TERM == "dumb" ];
        then
            if [ "$(alert --warning "Missing file: $file. Please copy original 'Heart of Darkness' files to $DATA_PATH." "Open data folder" "Cancel")" == "Open data folder" ];
            then
                open $DATA_PATH
            fi
        else
...
hode --datapath=$DATA_PATH --savepath=$SAVE_PATH $@